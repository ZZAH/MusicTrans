...
class Trainer:
    def __init__(self,  args):
        ...
        if args.checkpoint:
            checkpoint_args_path = os.path.dirname(args.checkpoint) + '/pretrained_musicnet/bestmodel_0.pth'
            print(checkpoint_args_path)
            checkpoint_args = torch.load(checkpoint_args_path)
            print(checkpoint_args)

            self.start_epoch = 0 #checkpoint_args[-1] + 1
            args.checkpoint = os.path.dirname(args.checkpoint) + '/pretrained_musicnet/bestmodel_0.pth'
            states = torch.load(args.checkpoint)
            print(states.keys())
            #states = torch.load(checkpoint_args_path)
    ...
    def evaluate_epoch(self, epoch):
        ...
        with tqdm(total=n_batches) as valid_enum, \
            ...
            for batch_num in range(n_batches):
                ...
                self.test_loss_list.append(batch_loss)
    ...
    def train(self):
        ...
        np.save("train_loss.npy",np.array(self.train_loss_list))
        np.save("test_loss.npy", np.array(self.test_loss_list))
